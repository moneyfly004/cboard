      ...
      TaskEither.tryCatch(
        ...
        try {
          if (profEntity != null && profEntity is RemoteProfileEntity) {
            ...
            if (active) {
              profEntity = profEntity.copyWith(active: true);
            }
                  ...
                  (profEntity) =>
                      validateConfig(
                        ...
                        (unit) => TaskEither.tryCatch(() async {
                          final nextEntry = active ? profEntity.copyWith(active: const Value(true)) : profEntity;
                          await _profileDataSource.edit(id, nextEntry);
                          return unit;
                        }, ProfileFailure.unexpected),
                      ),
                );
                ...
                .flatMap(
                  (profEntity) =>
                      validateConfig(
                        ...
                        (unit) => TaskEither.tryCatch(() async {
                          final nextEntry = active ? profEntity.copyWith(active: const Value(true)) : profEntity;
                          await _profileDataSource.insert(nextEntry);
                          return unit;
                        }, ProfileFailure.unexpected),
                      ),
                );
